...
Despite this progress, large-scale studies frequently neglect endangered languages (\citealt{hamalainen2021endangered}) and systematic performance disparities in multilingual models (see \citealt{shani2026rootsperformancedisparitymultilingual, chelombitko2024qtokcomprehensiveframeworkevaluating, dunn2011evolved}). We address this gap by examining 242 languages\footnote{The number 242 refers to the tokenizer-training set (205 Latin + 37 Cyrillic Wikipedia dumps that meet the corpus-size threshold). The E1 evaluation (\S\ref{sec:e1}) uses a larger 321-language Latin-script test bed for language identification; the relation between the two counts is explained in \S\ref{sec:glottoset}.} through a novel script-level lens (Latin vs.\ Cyrillic), moving beyond family-specific comparisons (\citealt{meillet1967comparative, beekes2011comparative}) to reveal overarching patterns visible only when languages share a script-based writing system (\citealt{daniels2007world, rust-etal-2021-good}).
...
\paragraph{Language counts used throughout the paper.} Two figures appear in the paper's headline results and refer to related but distinct scopes; readers are encouraged to keep them separate:

\begin{itemize}[noitemsep,topsep=0.2em,leftmargin=1.4em]
\item \textbf{242} = tokenizer-training set: the subset of the Latin+Cyrillic dumps that met a per-language corpus-size threshold for training a stable BPE tokenizer. It splits as 205 Latin-script + 37 Cyrillic-script. Everything in \S\ref{sec:vectors} (rank-based subword vectors), E2 (\S\ref{sec:e2}), E3 (\S\ref{sec:e3}) and E4 (\S\ref{sec:e4}) uses this set. Table~\ref{tab:data_stats} shows the top 30 by corpus size.
\item \textbf{321} = Latin-script E1 test bed: all Latin-script Wikipedia dumps, including those below the tokenizer-training threshold. E1 (\S\ref{sec:e1}) only needs to sample test words from each language, not to train a tokenizer on it, so the Latin-script test bed can be larger than the training set. The 116 languages in $321 \setminus 205$ are Latin-script Wikipedias whose corpora are too small to train a stable tokenizer but still large enough to sample 500 alphabetic test words; each such word is tokenized by all 205 trained Latin-script tokenizers.
\end{itemize}

The Cyrillic-script E1 test bed is the same 37 languages as the Cyrillic training set, because we retained every Cyrillic Wikipedia dump that passed the paragraph filter. Evaluations on Latin and Cyrillic sets are reported separately (\S\ref{sec:e1}); Table~\ref{tab:e1_summary} reports 320 rather than 321 because one Latin-script dump did not yield enough alphabetic test words to sample.
...
\textbf{Baselines.} We compare against two references: (i) a random-guessing baseline over the label alphabet (0.31\% on 321 Latin-script languages), and (ii) fastText \texttt{lid.176}~\citep{joulin2016bag}, a supervised character-\textit{n}-gram classifier that is the standard off-the-shelf language identification tool. fastText's label alphabet contains 176 languages; on the 320 Latin-script dumps that yielded enough test words (321 Latin minus one dump for which the word-sampling filter left too few candidates), it produces the correct label at least once for only 132 languages, and returns zero recall for the remaining 188. That gap is precisely the space our method is intended to cover.

\textbf{Results:} Table~\ref{tab:e1_summary} reports overall accuracy on the 320 Latin-script test dumps. Our BPE-token-count method reaches 13.8\%; fastText reaches 19.83\% overall on the same set but returns 0\% for 147 of them because those languages are outside its label alphabet, and reaches 38.42\% on the 132-language subset it does cover, dominating our method as expected of a supervised system with in-domain labels. Table~\ref{tab:e1_headline_langs} gives the head-to-head on eight illustrative languages: the four best performers of our method (Lak, Cree, Inuktitut, Kabardian) all sit at 0\% under fastText because they are not in \texttt{lid.176}; conversely, high-resource, script-marked languages such as Greek, Hebrew, Korean, and Russian sit near-ceiling under fastText and are cases where our BPE-count method has no comparative advantage.

\begin{table}[ht]
\caption{E1 overall accuracy on the 320 Latin-script Wikipedia dumps that yielded enough alphabetic test words (up to 500 words $\geq$5 chars per language). The BPE token-count method has full coverage across the 320-language label alphabet; fastText \texttt{lid.176} returns zero for languages outside its 176-language label set.}
