...
\author{
  \textbf{Iaroslav Chelombitko}\textsuperscript{1,2,3},\;
  \textbf{Mika H\"am\"al\"ainen}\textsuperscript{2},\;
  \textbf{Aleksei Komissarov}\textsuperscript{1}\\[0.6em]
  \begin{tabular}{ccc}
    \textsuperscript{1}Neapolis University Pafos & \textsuperscript{2}Metropolia University of Applied Sciences & \textsuperscript{3}DataSpike \\
    Pafos, Cyprus & Helsinki, Finland & Limassol, Cyprus \\
  \end{tabular}\\[0.4em]
  \small
  \texttt{i.chelombitko@nup.ac.cy}\quad
  \texttt{mika.hamalainen@metropolia.fi}\quad
...
Despite this progress, large-scale studies frequently neglect endangered languages (\citealt{hamalainen2021endangered}) and systematic performance disparities in multilingual models (see \citealt{shani2026rootsperformancedisparitymultilingual, chelombitko2024qtokcomprehensiveframeworkevaluating, dunn2011evolved}). We address this gap by examining 242 languages\footnote{The number 242 refers to the tokenizer-training set (205 Latin + 37 Cyrillic Wikipedia dumps that meet the corpus-size threshold). The E1 evaluation (\S\ref{sec:e1}) uses a larger 320-language Latin-script test bed for language identification; the relation between the two counts is explained in \S\ref{sec:glottoset}.} through a novel script-level lens (Latin vs.\ Cyrillic), moving beyond family-specific comparisons (\citealt{meillet1967comparative, beekes2011comparative}) to reveal overarching patterns visible only when languages share a script-based writing system (\citealt{daniels2007world, rust-etal-2021-good}).
...
We processed Wikipedia dumps from the ZIM archive~\footnote{\url{https://dumps.wikimedia.org/kiwix/zim/wikipedia/}}. The pipeline extracted clean paragraph text by resolving redirects and stripping auxiliary HTML tags. To ensure data quality, we filtered paragraphs by script consistency (retaining only Latin or Cyrillic characters) and excluded segments containing fewer than 10 words to reduce noise from accidental cross-lingual fragments. After deduplicating to unique ISO language codes, 320 dumps are dominantly Latin-script and 37 are dominantly Cyrillic-script; a further set of dumps in other scripts (Han, Arabic, Devanagari, etc.) is analyzed for cross-script contamination in Appendix Table~\ref{tab:additional_languages}.
...
\paragraph{Language counts at a glance.} Several distinct language totals appear in the paper. Table~\ref{tab:language_counts} lists them in one place.

\begin{table}[ht]
\caption{Language counts used in this paper. Rows are indexed by the number as it appears in the text; the rightmost column lists where the number is used.}
\label{tab:language_counts}
\centering
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}rp{5.2cm}l@{}}
\toprule
\textbf{N} & \textbf{What it is} & \textbf{Used in} \\
\midrule
\textbf{242} & \textbf{Tokenizer-training set.} Latin+Cyrillic Wikipedia dumps that met the per-language corpus-size threshold for training a stable BPE tokenizer. \emph{Splits as 205 Latin + 37 Cyrillic.} & \S\ref{sec:vectors}, E2, E3, E4; Table~\ref{tab:data_stats} \\
205 & Latin-script subset of 242 (tokenizer-trained). & — \\
37 & Cyrillic-script subset of 242 (tokenizer-trained; also the Cyrillic E1 test bed). & E1 (Cyrillic) \\
\textbf{320} & \textbf{Latin-script E1 test bed.} All Latin-script Wikipedia dumps with unique language codes; the extra 115 beyond the 205 trained have corpora too small to train a stable tokenizer but big enough to sample test words. Test words are tokenized by all 205 trained Latin tokenizers. & E1 method (\S\ref{sec:e1}); Table~\ref{tab:e1_summary} \\
115 & = $320-205$: Latin dumps used only for sampling test words in E1. & — \\
279 & = $320-41$: Latin dumps for which fastText also had enough test words after the sampling filter (41 dumps had corpora too small). & fastText baseline \\
176 & fastText \texttt{lid.176} label alphabet size. & E1 baseline \\
132 & Subset of the 320 that fastText's alphabet covers with non-zero recall. & E1 baseline \\
147 & Latin dumps for which fastText returned zero correct labels because they are outside its 176-language alphabet. & E1 result \\
\bottomrule
\end{tabular}
\end{table}

The rest of \S\ref{sec:glottoset} follows the terminology in Table~\ref{tab:language_counts}. Latin and Cyrillic E1 evaluations are reported separately (\S\ref{sec:e1}); Appendix Table~\ref{tab:additional_languages} provides script-contamination analysis for a further 80 languages using non-Latin/non-Cyrillic scripts.
...
\textbf{Method:} For each test word from language $L$, we tokenized it with all available language-specific tokenizers and predicted $\hat{L} = \arg\min_{L'} |T_{L'}(w)|$, where $T_{L'}(w)$ is the tokenization of word $w$ by language $L'$'s tokenizer. We evaluated separately on Cyrillic-script (37 languages) and Latin-script (320 languages) using up to 500 test words per language (minimum 5 characters, lowercase alphabetic, sampled uniformly from the paragraph-cleaned Wikipedia dumps described in \S\ref{sec:glottoset}).

\textbf{Baselines.} We compare against two references: (i) a random-guessing baseline over the 320-language label alphabet (0.31\%), and (ii) fastText \texttt{lid.176}~\citep{joulin2016bag}, a supervised character-\textit{n}-gram classifier that is the standard off-the-shelf language identification tool. fastText's label alphabet contains 176 languages; on the 279 Latin-script dumps for which our sampling filter (minimum 5 characters, lowercase alphabetic, sampled from paragraph-cleaned Wikipedia) yielded at least one valid test word, it produces the correct label at least once for only 132 languages and returns zero recall for the remaining 147. That gap is precisely the space our method is intended to cover.

\textbf{Results:} Table~\ref{tab:e1_summary} reports overall accuracy. Our BPE-token-count method reaches 13.8\% averaged over 320 Latin-script languages. Under the same word-sampling protocol, fastText was applicable to 279 of the 320 dumps (the other 41 had corpora too small to sample the required number of alphabetic test words), where it reached 19.83\% overall, but zero recall on 147 of those 279 because they are outside its 176-language label alphabet; on the 132-language subset it does cover it reached 38.42\%, dominating our method as expected of a supervised system with in-domain labels. Table~\ref{tab:e1_headline_langs} gives the head-to-head on eight illustrative languages: the four best performers of our method (Lak, Cree, Inuktitut, Kabardian) all sit at 0\% under fastText because they are not in \texttt{lid.176}; conversely, high-resource, script-marked languages such as Greek, Hebrew, Korean, and Russian sit near-ceiling under fastText and are cases where our BPE-count method has no comparative advantage.

\begin{table}[ht]
\caption{E1 overall accuracy. Coverage $x/y$ = the method returned a valid prediction for $x$ of the $y$ dumps in its evaluation slice. BPE is evaluated over the full 320-language Latin-script test bed with unique language codes; fastText is evaluated over the 279 dumps for which the sampling filter yielded at least one valid test word (41 dumps had corpora too small to sample under the minimum-5-character, lowercase-alphabetic filter). fastText reaches non-zero recall on the 132-language subset covered by its \texttt{lid.176} label alphabet.}
...
fastText \texttt{lid.176} & 132 / 279 & 19.83\% \\
