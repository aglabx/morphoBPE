...
\maketitle
\begin{abstract}
We release \textsc{morphoBPE}, a framework for training and comparing per-language Byte-Pair Encoding (BPE) tokenizers across 242 Latin- and Cyrillic-script languages built from Wikipedia \emph{glottosets} (per-language TF-DF lexicons), together with four diagnostic evaluations that probe what BPE captures across scripts. On unsupervised language identification via token count, our method reaches 13.8\% accuracy across 321 Latin-script languages (random 0.31\%); the supervised fastText \texttt{lid.176} baseline reaches 19.83\% on the same 320-language test set but returns 0\% for 147 of them because they fall outside its 176-language label alphabet, including our best-performing typologically distinctive languages (Lak 81.5\%, Cree 80.6\%, Inuktitut 63.8\%, Kabardian 60.1\%, all 0\% under fastText). BPE segmentation aligns with morpheme boundaries at F1 = 0.28 across 15 languages with derivational annotations (random 0.13), below the F1 = 0.43 of the unsupervised Morfessor baseline; BPE pre-training on target-language Wikipedia beats supervised character-\textit{n}-gram boundary classifiers trained on a related source language by +37\% F1 across six cross-lingual pairs, where labeled morphological data is unavailable. Pairwise BPE distance correlates moderately with Glottolog phylogeny (Mantel $r = 0.329$, $p < 0.001$; Romance forms the tightest cluster, mean distance 0.51). Across 26{,}939 Slavic Cyrillic homographs, 48.7\% receive different segmentations across related languages, correlating with linguistic distance. We position per-language BPE as a scalable \emph{diagnostic} tool for macro-linguistic comparison across languages lacking labeled morphological data, not as a substitute for supervised methods where such data exists.
\end{abstract}
...
Our approach carries five limitations that shape how the results should be read.
...
\textbf{Custom tokenizer.} We do not compare against SentencePiece Unigram or WordPiece under the same corpora; we release the tokenizer and glottosets to enable that comparison in follow-up work.

\textbf{No comparison to mBERT-family multilingual tokenizers.} mBERT, XLM-R, and mT5 tokenizers are trained jointly across languages under different training corpora and objectives; matching those setups was out of scope here. Comparing our per-language tokenizers against a subword vocabulary jointly learned across the same 242 languages is left to future work; \citet{rust-etal-2021-good} suggest such a comparison would primarily illuminate the value of per-language vocabulary allocation.

Beyond addressing these limitations, planned work applies this framework to Common Crawl for expanded coverage, compares BPE distances against typological databases WALS~\citep{wals} and Grambank~\citep{grambank}, and controls for word frequency, as suggested by \S\ref{sec:e4}, to separate frequency effects from morphological differences.
...
The reproducible code, together with the trained per-language BPE tokenizers, the evaluation scripts for E1--E4 including the fastText comparison of \S\ref{sec:e1}, and the merge-graph visualisation, is available at \url{https://github.com/aglabx/morphoBPE} under the MIT licence. The Wikipedia glottosets (per-language TF-DF word lists) are released on Hugging Face at \url{https://huggingface.co/datasets/aglabx/wiki_glottosets} under CC BY-SA.

\section{AI Models Usage}

As non-native English speakers, we used Claude Opus 4.5 for text editing. GitHub Copilot assisted with code completion. For research automation, we used Claude Code integrated with html-based reports for experimental pipelines and iterative analysis. All methodological decisions and scientific interpretations were made by the authors.
